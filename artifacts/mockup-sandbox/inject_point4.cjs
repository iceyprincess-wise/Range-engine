...
// Safe pathing for strict ESM environments
const targetDir = path.join(process.cwd(), 'src', 'components', 'mockups', 'range-engine');
const targetFile = path.join(targetDir, 'InjuryVacuumEngine.tsx');
...
try {
    ...
    console.log('\n\x1b[32m%s\x1b[0m', '✅ SUCCESS: InjuryVacuumEngine.tsx generated flawlessly.');
} catch (err) {
    console.error('\n\x1b[31m%s\x1b[0m', '❌ ERROR WRITING FILE:', err);
}
